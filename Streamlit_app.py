...
pio.templates['TLO'] = go.layout.Template(
    layout=go.Layout(font=dict(family='Rockwell', size=16), title_font=dict(size=24), colorway=px.colors.qualitative.G10
    )
)
...
#call the data load function and get data into the 6 dataframes below    
...
#add some space after the first line of controls on the streamlit app screen
#st.markdown('##')
st.text(" ")
st.text(" ")
...
fig_dist = ff.create_distplot(
        hist_data, group_labels, colors=px.colors.qualitative.G10[1:])
...
#render the feature importance plot
...
''')
Report.markdown('''We first performed Principle Component Analysis on the COI data, then clustered based on the first 11 components.  We chose 11 because those components explained 80% of the variance in the data.  PCA is an orthogonal transformation of high dimensional data into a series of uncorrelated data in a reduced PCA space such that the first component explains the most variance in the data with each subsequent component explaining less.  The COI data has 31 dimensions and we used PCA to tackle the curse of dimensionality; too many dimensions (features) in the data causes noise and data processing challenges (when features have different scales, as an example). The first few components of PCA explain higher variance which in turn helped us visualize and validate the results of clustering methods on a 2D scale.
''')
...
Report.markdown('''Our predictor variables were the COI indicators: population levels (both children-only and total population); the SEDA year, grade level, and subject (reading or math); and the cluster label resulting from the K-Means model.  We chose the mean score for all students as the target variable from the SEDA dataset for our prediction tasks.  Using this target, we tried several prediction methods from the scikit-learn package, outlined in the results table below, including linear, tree-based, and histogram-based models (Pedregosa et al., 2011).  Once the most promising model was selected, we performed hyperparameter tuning cross-validation to optimize model performance.
''')
...
''')
Report.markdown('''We discovered that one of the resulting clusters was very small (4 school districts), but consisted of 4 enormous metropolitan districts (New York, Los Angeles, Dade County (Miami), and Chicago). The other 3 clusters were both more diverse and closer in size with 4710, 2539, and 3594 school districts, respectively.  The below plot shows the clusters based on the first 2 PCA components.  As can be seen, the small cluster (Cluster 3), is in the upper right of the plot.  The remaining clusters are densely packed.  This density is what caused the DBSCAN clustering algorithm to be ineffective, just finding 1 cluster and many outliers.  This visualization uses log-scale axes to spread out the tightly grouped clusters to make the divisions more visible. 
''')
...
Report.markdown('''Overall, the histogram-based gradient boosted tree regression method produced a similar score on the training set to the more common, non-histogram gradient boosted tree regression, but performed much better, efficiency-wise, on over 150,000 rows of training data.  Both of these regression techniques are based on ensembles of decision trees executed in succession, where each tree learns from its predecessors according to a selected loss function and learning rate.  The histogram variant from scikit-learn is optimized for large datasets (10,000+ rows) and operates by grouping continuous variables into integer bins (Pedregosa et al., 2011).  These bins can be processed by the tree much more efficiently than the ungrouped continuous values.  This better performance allowed us to do a grid search with cross-validation of learning rate and maximum depth parameters, leading to an improved score on random subsets of the training data.  Cross-validation result details are in the Appendix.
''')
Report.markdown('''Once we decided on the histogram-based gradient boosting tree regressor method, we trained models both on all the training data at once, and also as separate models for each cluster.  Grid search resulted in different hyperparameters for the cluster models.  The clusters also had widely varying $R^2$ scores on the training data and the test set.  The overall model was moderately successful and had correct looking residuals (plot in Appendix), but the cluster 1 performed much worse. 
''')
...
Report.markdown('''The features in the above distributions can be divided into two groups: positive factors that represent real-world advantages and negative factors that represent real-world disadvantages.  The pattern between clusters 1, 2, and 4 are consistent.  For positive factors – 3rd grade scores, health insurance coverage – Cluster 2 (higher scoring) shows higher values and Cluster 4 (lower scores) shows lower values.  For negative factors – poverty, pollution – the opposite is true.  In each case, Cluster 1 is roughly in the middle.
''')
...
''')
Report.markdown('''The dashboard we created based on this analysis can be used by parents and school administrators to identify districts that are doing well or poorly compared to other districts that have similar characteristics.  This information can be used to support improvements in school achievement by providing fairer comparisons between districts. 
''')
Report.markdown('''This project had several limitations, one of which was due to an ethical concern with the SEDA data.  While individual school-level data is available from SEDA, some of the values are obscured due to privacy concerns, particularly if the school has a small population of ethnic minority children.  To avoid this issue, we used data from the district level, which was not obscured.  Another limitation was choosing to predict just one of the many possible outcome variables in the SEDA data (overall mean score).
''')
...
''')
Report.markdown('''1. US Census tract-level total population - Used to scale the population for each tract to get an estimate of the population in the school district.
    ''')
Report.markdown('''2. School district geographic relationship files - Used to link census tracts to school districts.  Districts may have multiple tracts within them, and tracts may belong to multiple districts.  As described above in the Data Cleaning section, we used these relationship files and the total population both to establish the relationships and to create a weighted average for the whole school district.  
''')
Report.markdown('''3. School district shape files - Used to get latitude and longitude for each school district for plotting on a map on the dashboard.
''')
